  ...
  validateForm() {
    ...
    data_items.forEach((item) => {
      ...
      if (item.element === "TextInput") {
        const $item = this._getItemValue(item, this.inputs[item.field_name]);
        if ($item && $item.value) {
          // Check Pattern
          if (!!item.inputPattern) {
            const reg = new RegExp(item.inputPattern);
            if (!reg.test(String($item.value).toLowerCase().trim())) {
              errors.push(`${item.label} is invalid!`);
            }
          }
        }

        // Check Email
        if ($item.value && item.isEmail) {
          if (!this.validateEmailAddress($item.value.trim())) {
            errors.push(`${item.label} is invalid`);
          }
        }
      }

      if (item.element === "NumberInput" || item.element === "TextArea") {
        const $item = this._getItemValue(item, this.inputs[item.field_name]);
        ...
        // Check Phone number
        if ($item.value && item.isMobile) {
          const mobileRegex = new RegExp("^([0|+[0-9]{1,5})?([7-9][0-9]{9})");
          if (!mobileRegex.test($item.value)) {
            errors.push(`${item.label} is invalid`);
          }
        }
      }
